...
export interface SiteContent { meta: { title: string; description: string; lang: string }; navigation: NavigationItem[]; headerCta?: CTA; socialLinks: SocialLinkData[]; footer: { copyright: string } }
...
export function validateSiteContent(value: unknown): SiteContent {
  ...
  return { meta: { title: requiredString(meta.title, `${path}.meta.title`), description: requiredString(meta.description, `${path}.meta.description`), lang: requiredString(meta.lang, `${path}.meta.lang`) }, navigation: data.navigation.map((item, index) => { const itemPath = `${path}.navigation[${index}]`; const navItem = record(item, itemPath); return { label: requiredString(navItem.label, `${itemPath}.label`), href: requiredString(navItem.href, `${itemPath}.href`) }; }), headerCta: data.headerCta === undefined ? undefined : validateCta(data.headerCta, `${path}.headerCta`), socialLinks: data.socialLinks.map((item, index) => { const itemPath = `${path}.socialLinks[${index}]`; const link = record(item, itemPath); return { platform: requiredString(link.platform, `${itemPath}.platform`), url: requiredString(link.url, `${itemPath}.url`), ariaLabel: requiredString(link.ariaLabel, `${itemPath}.ariaLabel`) }; }), footer: { copyright: requiredString(footer.copyright, `${path}.footer.copyright`) } };
}
